...
class Admin{
	...
	public function update_user($username,$email, $fname, $lname, $cnumber){
		
		/* Setting Timezone for DB */
		$NOW = new DateTime('now', new DateTimeZone('Asia/Manila'));
		$NOW = $NOW->format('Y-m-d H:i:s');

		$sql = "UPDATE admin SET adm_fname=:fname, adm_lname=:lname, user_date_updated=:user_date_updated,user_time_updated=:user_time_updated, user_access=:user_access WHERE username=:username";

		$q = $this->conn->prepare($sql);
		$q->execute(array(':fname'=>$fname, ':user_lastname'=>$lname,':user_date_updated'=>$NOW,':user_time_updated'=>$NOW,':user_access'=>$access,':user_id'=>$id));
		return true;
	}
