...
class Customer{
	...
	public function new_customer($fname,$mname,$lname,$email,$address,$cnumber){
		
		$data = [
			[$fname,$mname,$lname,$email,$address,$cnumber],
		];
		$stmt = $this->conn->prepare("INSERT INTO customer (cust_fname, cust_mname, cust_lname, cust_email, cust_address, cust_cnumber) VALUES (?,?,?,?,?,?)");
		try {
	...
	public function update_customer($fname, $mname, $lname, $email, $address, $cnumber, $id){
		
		$sql = "UPDATE customer SET cust_fname=:cust_fname, cust_mname=:cust_mname, cust_lname=:cust_lname, cust_email=:cust_email, cust_address=:cust_address, cust_cnumber=:cust_cnumber WHERE cust_id=:cust_id";

		$q = $this->conn->prepare($sql);
		$q->execute(array(':cust_fname'=>$fname, ':cust_mname'=>$mname,':cust_lname'=>$lname,':cust_email'=>$email,':cust_address'=>$address, 'cust_cnumber'=>$cnumber, ':cust_id'=>$id));
		return true;
	}
